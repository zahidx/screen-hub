...
const ComedyMovies = () => {
    ...
    const fetchComedyMovies = async () => {
      try {
        const res = await fetch(
          `https://api.themoviedb.org/3/genre/35/movies?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&page=1`
        );
        const data = await res.json();
        setMovies(data.results);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch comedy movies');
        setLoading(false);
      }
    };
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8">
            {[...Array(12)].map((_, index) => (
              <div key={index} className="group relative bg-gray-700 rounded-lg shadow-lg animate-pulse">
                <div className="h-[300px] w-full bg-gray-500 rounded-lg"></div>
  ...
  return (
    <div className="bg-gradient-to-r from-[#1E2A47] to-[#4A6A92] min-h-screen py-12 px-8">
      <div className="text-center mb-12">
        <h1 className="text-5xl font-extrabold text-white mb-4">Comedy Movies</h1>
        <input
          type="text"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-8">
        {filteredMovies.slice(0, 12).map((movie) => (
          <div
            key={movie.id}
            className="group relative bg-gradient-to-br from-[#2A3B5C] to-[#1D2D45] rounded-lg shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-xl cursor-pointer"
          ...
          >
            <div className="relative h-[350px] w-full overflow-hidden rounded-lg">
              <img
                ...
                alt={movie.title}
                className="absolute top-0 left-0 right-0 bottom-0 object-cover w-full h-full transition-all duration-300 ease-in-out group-hover:scale-110 group-hover:opacity-80"
                loading="lazy"
              />
              <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-t from-black to-transparent opacity-70 group-hover:opacity-90 transition-all duration-300">
                <div className="absolute bottom-4 left-4 text-white">
