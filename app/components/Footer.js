...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      setShowScrollButton(true); // Show the button as soon as scrolling happens
  
      // Clear the previous timeout if any
      if (timeout) clearTimeout(timeout);
  
      // Set a new timeout to hide the button after 3 seconds of no scroll
      timeout = setTimeout(() => {
        setShowScrollButton(false); // Hide the button after 3 seconds of no scrolling
    ...
    return () => {
      ...
      if (timeout) clearTimeout(timeout); // Clear timeout on cleanup
    };
  }, []);
